...
>     - **可视化**：尝试集成 @antv/infographic → 部署红 → 调查发现 498KB obfuscated `preinstall` 含 crypto + 27 处 fetch（典型 phone-home），**4 commit 一次性回滚** → 换 Mermaid（社区透明、客户端 lazy load 600KB core + 按 diagram 按需 chunk）；新博文 [让博客学会画图](/blog/visualization-saga/) 记录全过程
